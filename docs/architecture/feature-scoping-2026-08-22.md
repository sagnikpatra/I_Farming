...
| 3 | Real-world-timezone weather | S/M (cosmetic, recommended) / L (mechanical, not recommended) | New device-timezone read dependency either way | **Built (2026-08-22, commit `81a0d3c` + a same-day stretch goal)** -- Option A (cosmetic day/night, S complexity); Option B (seasonal palette -- real-calendar-month tint layered on the phase preset) also shipped same-day; villager lamp-lighting (structure-adjacent light sources at Night) remains the one open stretch goal, deliberately separated out for its own asset/placement questions |
