...
| 4 | Richer ambient villager behavior | M (contingent) / L (if new animation sourcing needed) | Verify `Rig_Medium_General.glb` has a usable idle clip first | **Built (2026-08-22, commits `028c36a` + 2 same-day stretch goals)** -- prerequisite confirmed true (Idle_A/Idle_B exist), idle-pause shipped at M; congregating (villagers turn to face each other) and point-of-interest lingering (villagers bias toward decoration-adjacent tiles) both shipped as same-day stretch goals; night population thinning remains the only open stretch goal |
