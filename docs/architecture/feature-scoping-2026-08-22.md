...
| 4 | Richer ambient villager behavior | M (contingent) / L (if new animation sourcing needed) | Verify `Rig_Medium_General.glb` has a usable idle clip first | **Built (2026-08-22, commits `028c36a` + congregating stretch goal same day)** -- prerequisite confirmed true (Idle_A/Idle_B exist), idle-pause shipped at M; congregating (two idling villagers turn to face each other) shipped as a same-day stretch goal; decoration lingering and night population thinning remain open stretch goals |
